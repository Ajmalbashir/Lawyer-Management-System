...
if(isset($_POST['form1'])) {
    ...
    if($path!='') {
        ...
        if( $ext!='jpg' && $ext!='png' && $ext!='jpeg' && $ext!='gif' ) {
            $valid = 0;
            $error_message .= 'You must have to upload jpg, jpeg, gif or png file for attorney photo<br>';
        }
    } else {
    	$valid = 0;
        $error_message .= 'You must have to select a photo for attorney photo<br>';
    }
    ...
    if(empty($_POST['password'])) {
        ...
        $valid = 0;
        $error_message .= 'Password must contain both letters and numbers<br>';
    }

	if($valid == 1) {

	    // getting auto increment id
	    $statement = $pdo->prepare("SHOW TABLE STATUS LIKE 'tbl_attorney'");
	    $statement->execute();
	    ...
	    $final_name = 'attorney-'.$ai_id.'.'.$ext;
	    move_uploaded_file( $path_tmp, '../assets/uploads/'.$final_name );

	    $final_name1 = 'attorney-banner-'.$ai_id.'.'.$ext1;
	    move_uploaded_file( $path_tmp1, '../assets/uploads/'.$final_name1 );

	    // Hash the password
	    $hashed_password = password_hash($_POST['password'], PASSWORD_DEFAULT);

	    $statement = $pdo->prepare("INSERT INTO tbl_attorney (name,slug,designation_id,photo,banner,degree,detail,facebook,twitter,linkedin,youtube,google_plus,instagram,flickr,address,practice_location,phone,email,website,status,meta_title,meta_keyword,meta_description,password) VALUES (?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)");
	    ...
	    $success_message = 'Attorney is added successfully!';
...
<section class="content-header">
	...
	<div class="content-header-right">
		<a href="attorney.php" class="btn btn-primary btn-sm">View All</a>
	</div>
</section>
				...
				<div class="box box-info">
					<div class="box-body">
						...
						<div class="form-group">
							...
							<div class="col-sm-6">
								<input type="text" autocomplete="off" class="form-control" name="email" required value="<?php if(isset($_POST['email'])){echo $_POST['email'];} ?>">
							</div>
						</div>
						<div class="form-group">
							<label for="" class="col-sm-2 control-label">Password <span>*</span></label>
							<div class="col-sm-6">
								<input type="password" autocomplete="off" class="form-control" name="password" required value="<?php if(isset($_POST['password'])){echo $_POST['password'];} ?>">
							</div>
						</div>
						<div class="form-group">
